...
import { CardRemoveButton } from "@/components/buttons/CardRemoveButton";
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col">
        ...
        <span className="text-md opacity-80">{artists?.join(", ")}</span>
      </div>
    </div>
  );
};
  ...
  useEffect(() => {
    ...
    if (currentSongData) {
      if (currentSongData.song === song) {
        audioRef.current.currentTime = localStorage.getItem("currentTime") ?? 0;
      }
    }
